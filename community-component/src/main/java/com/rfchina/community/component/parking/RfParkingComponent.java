 ...
 */
@Component
public class RfParkingComponent extends AbstractParkingComponent {

    private interface ApiPath {
        String GET_FEE = "/getFee";
        String GET_MONTHLY_INFO = "/getMonthlyInfo";
        String GET_REAL_TIME = "/getRealTime";
        String PAY_NOTIFY = "/payNotify";
        String PAY_MONTH_NOTIFY = "/payMonthNotify";
        String LIST_MONTHLY_INFOS = "/listMonthlyInfos";
    }

    public GetParkingFeeResponse getParkingFee(String host, GetParkingFeeRequest request) {
        String url = host + ApiPath.GET_FEE;
        try {
            String rspJsonStr = jsonGet(url, request);
    ...
    public ListMonthlyInfosResponse listMonthlyInfos(String host, ListMonthlyInfosRequest request) {
        String url = host + ApiPath.LIST_MONTHLY_INFOS;
        try {
            String rspJsonStr = jsonGet(url, request);
            logger.info("Invoke[listMonthlyInfos], response: {}", rspJsonStr);
            if(StringUtils.isNotBlank(rspJsonStr)) {
                ListMonthlyInfosResponse rsp = OBJECT_MAPPER.readValue(rspJsonStr, ListMonthlyInfosResponse.class);
                return rsp;
            }
        } catch (IOException e) {
            logger.error("Invoke listMonthlyInfos error", e);
        }
        return null;
    }
