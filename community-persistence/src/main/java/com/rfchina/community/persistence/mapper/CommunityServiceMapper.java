...
public interface CommunityServiceMapper extends Serializable {
    @SelectProvider(type=CommunityServiceSqlProvider.class, method="countByExample")
    long countByExample(CommunityServiceExample example);

    @DeleteProvider(type=CommunityServiceSqlProvider.class, method="deleteByExample")
    int deleteByExample(CommunityServiceExample example);
    ...
    @Insert({
        "insert into community_service (id, title, ",
        "png, disable_png, ",
        "reorder, clause, ",
        "type, is_account, ",
        "link, provide_type, ",
        ...
        "#{png,jdbcType=VARCHAR}, #{disablePng,jdbcType=VARCHAR}, ",
        "#{reorder,jdbcType=INTEGER}, #{clause,jdbcType=VARCHAR}, ",
        "#{type,jdbcType=INTEGER}, #{isAccount,jdbcType=INTEGER}, ",
    ...
    @InsertProvider(type=CommunityServiceSqlProvider.class, method="insertSelective")
    int insertSelective(CommunityService record);

    @SelectProvider(type=CommunityServiceSqlProvider.class, method="selectByExample")
    @Results({
    ...
    List<CommunityService> selectByExample(CommunityServiceExample example);
    ...
    CommunityService selectByPrimaryKey(Integer id);

    @UpdateProvider(type=CommunityServiceSqlProvider.class, method="updateByExampleSelective")
    int updateByExampleSelective(@Param("record") CommunityService record, @Param("example") CommunityServiceExample example);

    @UpdateProvider(type=CommunityServiceSqlProvider.class, method="updateByExample")
    int updateByExample(@Param("record") CommunityService record, @Param("example") CommunityServiceExample example);

    @UpdateProvider(type=CommunityServiceSqlProvider.class, method="updateByPrimaryKeySelective")
    ...
    @Select("SELECT t.title, t.description, c.`name` AS categoryName, c1.`name` AS categoryName2 " +
            "FROM community_service t " +
            "LEFT JOIN open_service_detail d ON t.id = d.service_id " +
            "LEFT JOIN community_service_category c ON c.id = t.category_id " +
            "LEFT JOIN community_service_category c1 ON c1.id = d.category_second_type " +
            "WHERE t.merchant_id = #{merchantId,jdbcType=INTEGER}")
    List<Map> selectByMerchantId(@Param("merchantId") Long  merchantId);
}