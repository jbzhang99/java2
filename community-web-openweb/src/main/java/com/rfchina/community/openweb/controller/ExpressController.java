...
public class ExpressController extends BaseController{
    ...
    @GetMapping(value = "/getExpressAuthorizeUrl")
    public void getExpressAuthorizeUrl(String companyCode, String expressNo, HttpServletResponse response){
        String url = expressCompanyService.getExpressAuthorizeUrl(companyCode,expressNo);
        if(StringUtils.isNotBlank(url)) {
            try {
                response.sendRedirect(url);
            } catch (IOException e) {
                logger.error("获取物流信息，重定向异常",e);
            }
        }
    }
