...
public class GlobalExceptionHandler {
	...
	public Result<?> jsonErrorHandler(HttpServletRequest request, Exception e) throws Exception {
		if (e instanceof MethodArgumentTypeMismatchException) {
			...
			return Result.create(StatusCode.FAILURE_INFO.value(), e.getMessage());
		} else if (e instanceof DisabledAccountException) {
			return Result.create(StatusCode.UNAUTHORIZED.value(), StatusCode.UNAUTHORIZED.name());
		}else if (e instanceof UnknownAccountException) {
			return Result.create(StatusCode.USER_IS_NOT_EXISTED.value(), StatusCode.USER_IS_NOT_EXISTED.name());
		}else if (e instanceof AuthenticationException) {
			return Result.create(((TempTokenInvalidException) e).getCode(), e.getMessage());
		}
